 ...
 * 
 * 使用方法:
 *   node scripts/database-init-optimized.js init    - 初始化数据库
 *   node scripts/database-init-optimized.js reset   - 重置数据库
 *   node scripts/database-init-optimized.js verify  - 验证数据库
 *   node scripts/database-init-optimized.js clean   - 清理数据库
 */
...
const createTableSQLs = {
  ...
  dining_orders: `
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='报餐记录表'
  `,
...
const sampleData = {
  ...
  venues: [
    ...
    },
    {
      ...
      price: 200.00,
      description: '大型会议室',
      facilities: JSON.stringify(['投影仪', '音响', '白板'])
    }
  ],
  ...
  dining_tables: [
    ...
    },
    {
      ...
      location: 'B区',
      maxCapacity: 8,
      qrCode: 'TABLE_B002_QR',
      verificationCode: 'B002V'
    },
    {
      _id: uuidv4(),
      tableName: 'C区01号桌',
      ...
      location: 'C区',
      maxCapacity: 10,
      qrCode: 'TABLE_C001_QR',
      verificationCode: 'C001V'
    }
  ],
  ...
  system_configs: [
    ...
    {
      _id: uuidv4(),
      configKey: 'dining.max_people',
      configValue: '20',
      dataType: 'number',
      category: 'dining',
      description: '单次报餐最大人数',
      isPublic: true
    },
    {
      _id: uuidv4(),
    ...
    {
      _id: uuidv4(),
      configKey: 'reservation.advance_hours',
      configValue: '24',
      dataType: 'number',
      category: 'reservation',
      description: '场地预约提前时间(小时)',
      isPublic: true
    },
    {
      _id: uuidv4(),
    ...
    {
      _id: uuidv4(),
      configKey: 'verification.code_length',
      configValue: '6',
      dataType: 'number',
      category: 'verification',
      description: '验证码长度',
    ...
    {
      _id: uuidv4(),
      configKey: 'jwt.expire_time',
      configValue: '7d',
      dataType: 'string',
      category: 'jwt',
      description: 'JWT Token过期时间',
      isPublic: false
    },
    {
      _id: uuidv4(),
      configKey: 'file.max_size',
...
async function insertSampleData(connection) {
  try {
    ...
    logger.info(`✅ 部门示例数据插入完成 (${sampleData.departments.length} 条)`);
    ...
    logger.info(`✅ 场地示例数据插入完成 (${sampleData.venues.length} 条)`);
    ...
    logger.info(`✅ 餐桌示例数据插入完成 (${sampleData.dining_tables.length} 条)`);
    ...
    logger.info(`✅ 系统配置示例数据插入完成 (${sampleData.system_configs.length} 条)`);
...
async function verifyDatabase(connection) {
  try {
    ...
    const [indexes] = await connection.query(`
      ...
      WHERE TABLE_SCHEMA = ? AND TABLE_NAME IN (${expectedTables.map(() => '?').join(',')})
      ORDER BY TABLE_NAME, INDEX_NAME
    `, [config.database.database, ...expectedTables]);
    
    logger.info(`✅ 索引统计: ${indexes.length} 个索引`);
  ...
  try {
    switch (command) {
      ...
      case 'reset':
        ...
        await createTables(connection);
        await insertSampleData(connection);
        const resetValid = await verifyDatabase(connection);
        if (resetValid) {
          logger.info('🎉 数据库重置完成！');
        } else {
      ...
      case 'verify':
        connection = await mysql.createConnection(config.database);
        const verifyValid = await verifyDatabase(connection);
        process.exit(verifyValid ? 0 : 1);
        break;
      ...
      default:
        console.log('智慧物业管理系统 - 优化数据库初始化脚本');
        console.log('');
        console.log('使用方法:');
        console.log('  node scripts/database-init-optimized.js init    - 初始化数据库（创建数据库、表和示例数据）');
        console.log('  node scripts/database-init-optimized.js reset   - 重置数据库（删除所有数据后重新初始化）');
        console.log('  node scripts/database-init-optimized.js verify  - 验证数据库结构');
        console.log('  node scripts/database-init-optimized.js clean   - 清理数据库（删除所有表）');
        console.log('');
