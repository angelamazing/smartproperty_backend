...
class DishService {
    ...
    try {
      const sql = `
        UPDATE dishes SET 
          ...
          tags = ?, status = ?, isRecommended = ?, updateTime = NOW(), updateBy = ?
        WHERE _id = ?
      `;
  ...
  async deleteDish(db, dishId, userId) {
    try {
      const sql = `
        UPDATE dishes SET 
          status = 'deleted', updateTime = NOW(), updateBy = ?
        WHERE _id = ?
      `;
  ...
  async batchUpdateDishStatus(db, dishIds, status, userId) {
    try {
      const sql = `
        UPDATE dishes SET 
          status = ?, updateTime = NOW(), updateBy = ?
